...
const controlRecipes = async function() {
    try {
        ...
        // 2. loading recipe
        await _modelJs.loadRecipe(id); //return promise so await
        // 3. Rendering recipe
...
const controlSearchResults = async function() {
    try {
        // 1. Get search query
        const query = _searchViewJsDefault.default.getQuery(); //NOTE function 中 會呼叫  #clearInput() 清掉spinner
        if (!query) return;
        // NOTE 這邊把renderSpinner下移更改了順序 來改掉原版本的BUG: search button with no value, the spinner will continue spin.
        _resultsViewJsDefault.default.renderSpinner(); //確定有query才執行 如果放在上面的 guard clause 之前的話 會變成spinner不會被消掉 因為return了
...
const init = function() {
    _bookmarksViewJsDefault.default.addHandlerRender(controlBookmarks); //299 fix local storage bug: 不然update的時候比較節點陣列會不同
    _recipeViewJsDefault.default.addHandlerRender(controlRecipes);
    _recipeViewJsDefault.default.addHandlerUpdateServings(controlServings);
    _recipeViewJsDefault.default.addHandlerBookmark(controlAddBookmark); //297
    _searchViewJsDefault.default.addHandlerSearch(controlSearchResults);
    _paginationViewJsDefault.default.addHandlerClick(controlPagination);
    _addRecipeViewJsDefault.default.addHandlerUpload(controlAddRecipe); //301
    newFeature();
};
...
const deleteBookmark = function(id) {
    ...
    ); // find index and use for delete later
...
const uploadRecipe = async function(newRecipe) {
    try {
        ...
        const ingredients = Object.entries(newRecipe).filter((entry)=>entry[0].startsWith('ingredient') && entry[1] !== ''
            ...
            ); //改掉有空格的bug e.g. chocolate milk 會變成 chocolatemilk 因此每個各別 trim()
...
},{"./config.js":"beA2m","./helper.js":"aqMTk","@parcel/transformer-js/src/esmodule-helpers.js":"JacNc"}],"beA2m":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class RecipeView extends _viewJsDefault.default {
    ...
    addHandlerUpdateServings(handler) {
        this._parentElement.addEventListener('click', function(e) {
            ...
            if (+updateTo > 0) handler(+updateTo); // guard 小於0 並且 轉成數字
        });
    }
...
class View {
   ...
   */ render(data, render = true) {
        ...
        // if there is data but that data is an array and it is empty
    ...
    update(data) {
        this._data = data;
        const newMarkup = this._generateMarkup(); //generate new markup and then compare with old(current) markup
        const newDOM = document.createRange().createContextualFragment(newMarkup); //DOM in the memory
        // NOTE node list could use forEach, but just in case we wanna use other array methods in the future
        const newElements = Array.from(newDOM.querySelectorAll('*')); //convert nodeList to real array
        const curElements = [
            ...this._parentElement.querySelectorAll('*')
        ]; //convert nodeList to real array with spread
...
class AddRecipeView extends _viewJsDefault.default {
    ...
    addHandlerUpload(handler) {
        this._parentElement.addEventListener('submit', function(e) {
            ...
            const dataArr = [
                ...new FormData(this)
            ]; //since we are in handler function, so 'this' pints to this._parentElement(upload form) (which the handler function attached to )
